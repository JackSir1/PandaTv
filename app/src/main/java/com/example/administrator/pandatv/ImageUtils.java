...
public class ImageUtils {
    ...
    public void setImageUrl(ImageView imageView,String url, int width, int height){
        ...
        if (url != null && !url.equals("null") && url.length() > 0) {
            if(width == 0 || height == 0){
                Picasso.with(mContext).load(url).placeholder(R.mipmap.ic_launcher).into(imageView);
            }else{
                Picasso.with(mContext).load(url).placeholder(R.mipmap.ic_launcher).centerCrop().into(imageView);
            }
        }
    }
    ...
    public void setHeadViewUrl(ImageView imageView, String url, ImageType type){
        ...
        if (url != null && !url.equals("null") && url.length() > 0) {
            Picasso.with(mContext).load(url).fit().placeholder(R.mipmap.ic_launcher).transform(transformation).into(imageView);
        }else{
            Picasso.with(mContext).load(R.mipmap.ic_launcher).fit().transform(transformation).into(imageView);
        }
    }
