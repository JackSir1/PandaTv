...
public class PandaVedioPlayer extends JCVideoPlayer {
    ...
    public  void setGetCallBack(GetOnClick getOnClick){
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start) {
            ...
            if (isCollect) {
                ...
                showToast("已取消收藏");
                isCollect = false;
                btn_collect.setBackgroundResource(R.drawable.wujiao1);
                //dosomething to not collect
            }else{
                //未收藏，点击收藏
                showToast("已添加，请到[我的收藏]中查看");
                isCollect = true;
                btn_collect.setBackgroundResource(R.drawable.wujiao2);
